...
export const compareAvgToPrevSeason = (data, prevSeasonData, entity, feature, racerType) => {
  ...
  const prevSeasonRacerData = prevSeasonData.filter((race) => race[colName] === entity);
  if (racerData.length === 0 || prevSeasonRacerData.length === 0) return "-";
  const currentSeasonSum =
    racerData.reduce((sum, race) => sum + race[feature], 0) / racerData.length;
  const prevSeasonSum =
    prevSeasonRacerData.reduce((sum, race) => sum + race[feature], 0) / prevSeasonRacerData.length;
  if (prevSeasonSum === 0) return "-";
  return ((100 * (currentSeasonSum - prevSeasonSum)) / prevSeasonSum).toFixed(2) + "%";
};
...
export const compareAvgToAllDrivers = (data, entity, feature, racerType) => {
  ...
  const othersAvgPasses =
    otherRacersData.reduce((sum, race) => sum + race[feature], 0) / otherRacersData.length;
...
export const getStagePointsPercentage = (data, entity, racerType) => {
  ...
  const allPoints = racerData.reduce(
    (sum, race) => sum + race.stage_points + race.finish_position_points,
    0
  );
  ...
  if (entityType === "driver") {
    if (seasonYear && entryList[seasonYear]) {
      entities = [
        ...new Set(
          raceData
            .filter((r) => entryList[seasonYear].includes(r.driver_name))
            .map((r) => r.driver_name)
        ),
      ];
    } else {
    ...
    entities = [
      ...new Set(
        raceData.map((r) => r.team_name).filter((name) => name.toLowerCase() !== "unknown")
      ),
    ];
  } else {
